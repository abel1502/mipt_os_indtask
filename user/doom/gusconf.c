...
//     DMXGUS lump into an equivalent Timidity configuration libc_FILE.
//
...
static void ParseLine(gus_config_t *config, char *line)
{
    ...
    instr_id = libc_atoi(fields[0]);
    mapped_id = libc_atoi(fields[MappingIndex()]);

    libc_free(config->patch_names[instr_id]);
    config->patch_names[instr_id] = libc_strdup(fields[5]);
...
static void FreeDMXConfig(gus_config_t *config)
{
    ...
    for (i = 0; i < MAX_INSTRUMENTS; ++i)
    {
        libc_free(config->patch_names[i]);
    }
}
...
static boolean WriteTimidityConfig(char *path, gus_config_t *config)
{
    ...
    libc_fprintf(fstream, "# Autogenerated Timidity config.\n\n");

    libc_fprintf(fstream, "dir %s\n", gus_patch_path);

    libc_fprintf(fstream, "\nbank 0\n\n");
...
boolean GUS_WriteConfig(char *path)
{
    ...
    {
        libc_printf("You haven't configured gus_patch_path.\n");
        libc_printf("gus_patch_path needs to point to the location of "
               "your GUS patch set.\n"
