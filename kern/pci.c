...
void
pci_print_device(unsigned bus, unsigned slot, unsigned func, uint8_t clid, uint8_t sclid, uint8_t hdrtype, uint16_t vendor, uint16_t device) {
	int found = 0;
	const char* vendor_name = pci_get_vendor_name(vendor, &found);
	if(found) {
		cprintf("%02u:%02u.%u %s: %s, device: 0x%4x\n", bus, slot, func, pci_get_device_type(clid, sclid), vendor_name, device);
	} else {
		cprintf("%02u:%02u.%u %s: Vendor 0x%04x, device: 0x%4x\n", bus, slot, func, pci_get_device_type(clid, sclid), vendor, device);
	}
}
	...
	for(unsigned bus = 0; bus < 256; bus++)
	{
		for(unsigned slot = 0; slot < 32; slot++)
		{
			...
			if(clid != 0xFF && vendor != 0xFFFF) {
				pci_print_device(bus, slot, func, clid, sclid, hdrtype, vendor, device);
			}
